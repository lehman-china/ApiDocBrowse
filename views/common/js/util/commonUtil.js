    ...
    convertToString: function (a) {
        ...
        if (a.constructor == String) {
            ...
            if (/type=/.test(a) && /explain=/.test(a)) {
                a = a.replace(/type=(.*?)& explain=(.*?)($|&).*/g, '<span style="color:green;font-weight:bold;">"{$1}"</span><span style="color:#808080;"> // $2</span>');
            } else {
        ...
        f = [];
        for (g in a) {
            if (g == "$$hashKey" || g == "explain" || g == "$explain") continue;
            // 值  value
            j = a[g];
            //变量名 key
            if (typeof j === 'object') {
        ...
        var objComm = '';
        if ("$explain" in a) {
            objComm = '<span style="color:#808080;"> // ' + a["$explain"] + '</span>';
        }
        return "{" + objComm + ju.n + f.join("," + ju.n) + ju.n + ju.__repeatStr(ju.t, b - 1) + "}"
    }, __isArray: function (a) {
        return a ? a.constructor == Array : !1
    }, __repeatStr: function (a, b) {
        var d, c = [];
...
Util.laytpl = function () {
    ...
    return e.pt = e.prototype, e.pt.parse = function (a, e) {
