...
mysql.exec( `SELECT * FROM ${tabName} limit 0,1`, [], function ( err, rows, fields ) {
    ...
    for ( var field of fields ) {
        ...
        // 编辑时 不显示字段
        var notEditCfg = "";
        if ( unshowEdit[ field.name ] ) {
            notEditCfg = " notEdit : true,";
        }

        routeCfg.push( `{ name : "${field.name}",title : "${field.name}", type : "${type}", width : "100", ${hideCfg + leftCfg + notEditCfg} query : true },` );
        keyIndex++;
